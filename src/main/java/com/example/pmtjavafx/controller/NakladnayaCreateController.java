...
public class NakladnayaCreateController {
    @FXML
    ...
    public void initialize() {
        ...
        count.setTextFormatter(formatter2);
        currencyRate.setTextFormatter(formatter3);
    }
    ...
    public void OnCreateButtonClick(ActionEvent event) {
        ...
        if (number.getText().isEmpty() || date.getValue() == null || user.getText().isEmpty() ||
                price.getText().isEmpty() || currency.getText().isEmpty() || currencyRate.getText().isEmpty() ||
                product.getText().isEmpty() || count.getText().isEmpty()) {
            errorLabel.setVisible(true);
            return;
        } else try {
            nakladnaya = new Nakladnaya(number.getText(), date.getValue(), user.getText(), Double.parseDouble(price.getText()),
                    currency.getText(), Double.parseDouble(currencyRate.getText()), product.getText(), Double.parseDouble(count.getText()));
        ...
        DocumentElement documentElement = new DocumentElement(gson.toJson(nakladnaya), new Document("Накладная", date.getValue().format(DateTimeFormatter.ofPattern("dd.MM.yyyy")), number.getText()));
